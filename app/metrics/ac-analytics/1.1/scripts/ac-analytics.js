require = function e(t, n, r) {
  ...
  158: [function(require, module, exports) {
    (function() {
      ...
      function getVisitNumPerChannel(sCode) {
        ...
        sCode.c_w("s_vnum_" + geoCode, "", 63072e3);
        sCode.c_w("s_invisit_" + geoCode, "", 63072e3);
        sCode.c_w("s_invisit_n_" + geoCode, "", 63072e3);
        sCode.c_w("s_vnum_n_" + geoCode, "", 63072e3);
        cl = sCode.c_r(c);
        cs = sCode.c_r(c2);
        if (channelMap) {
          var val;
  ...
  220: [function(require, module, exports) {
    ...
    var VALID_PARENT_NODE = [ac_dom_nodes.ELEMENT_NODE, ac_dom_nodes.DOCUMENT_NODE, ac_dom_nodes.DOCUMENT_FRAGMENT_NODE];
    var ERR_INVALID_PARENT_NODE = " must be an Element, Document, or Document Fragment";
    var VALID_CHILD_NODE = [ac_dom_nodes.ELEMENT_NODE, ac_dom_nodes.TEXT_NODE, ac_dom_nodes.COMMENT_NODE];
    var ERR_INVALID_CHILD_NODE = " must be an Element, TextNode, or Comment";
    var ERR_INVALID_SELECTOR = " must be a string";
    module.exports = {
      parentNode: function(node, required, funcName, paramName) {
  ...
  233: [function(require, module, exports) {
    ...
    proto._cleanStringData = function(arr) {
      ...
      var result = [],
        i, val, valLen, spaceIdx, len = arr.length;
      for (i = 0; i < len; i++) {
        val = arr[i];
    ...
    proto._getDelegateFuncBindingIdx = function(evt, delegateQuery, unboundCallback, context, ignoreFuncMatch) {
      var idx = -1;
      if (this._delegateFuncs[delegateQuery] && this._delegateFuncs[delegateQuery][evt]) {
        var i, bindingObj, len = this._delegateFuncs[delegateQuery][evt].length;
        for (i = 0; i < len; i++) {
    ...
    proto._triggerDelegateEvents = function(evt, delegateQuery, data) {
      ...
      var i, target, len = targets.length;
      for (i = 0; i < len; i++) {
        target = targets[i];
        if (document.createEvent) {
  ...
  238: [function(require, module, exports) {
    ...
    proto._setOptimizerEvents = function(optimizer) {
      var i, optimizerEvents = optimizer.eventNames,
    ...
    proto._setOptimizerProperties = function(optimizer) {
      var i, optimizeProperties = optimizer.propertyNames,
  ...
  242: [function(require, module, exports) {
    ...
    var resizeOptimizer = new Optimizer(config, function(e) {
      var i, propertyNames = config.propertyNames,
  ...
  244: [function(require, module, exports) {
    ...
    module.exports = [resizeEvt, scrollEvt]
  }, {
    "./events/resize": 242,
    "./events/scroll": 243
  }],
  ...
  306: [function(require, module, exports) {
    ...
    feature.isRetina = function() {
      var mediaQueryStrings = ["min-device-pixel-ratio:1.5", "-webkit-min-device-pixel-ratio:1.5", "min-resolution:1.5dppx", "min-resolution:144dpi", "min--moz-device-pixel-ratio:1.5"];
      var i;
  ...
  reNdTz: [function(require, module, exports) {
    (function() {
      ...
      function submit(request, metadata, sCode) {
        ...
        var img1x1;
        if (typeof sCode === "object") {
          src = _getBasePath(url) + (metadata.countryCodeFilter !== "us" ? metadata.countryCodeFilter : "") + "/b/ss/" + sCode.un + "/" + (sCode.mobile ? "5.1" : "1") + "/" + sCode.version + "/s0" + Date.now() + "?ndh=1&t=" + _getTimestamp() + "&fid=" + sCode.fid + "&g=" + url + "&pageName=" + metadata.pageName + "&cc=" + sCode.currencyCode + "&c3=" + title + "&h1=" + sCode.channel + "&pe=lnk_e&pev2=" + title + "&s=" + sCode.resolution + "&c=" + sCode.colorDepth + "&j=" + sCode.javascriptVersion + "&v=" + sCode.javaEnabled + "&k=" + sCode.cookiesEnabled + "&bw=" + sCode.browserWidth + "&bh=" + sCode.browserHeight + "&p=" + sCode.plugins + "&r=" + sCode.eVar49;
